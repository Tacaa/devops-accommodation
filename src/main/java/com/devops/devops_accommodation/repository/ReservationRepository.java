...
public interface ReservationRepository extends JpaRepository<Reservation, Integer> {
    ...
    @Query("SELECT r FROM Reservation r WHERE r.accommodation.hostId = :hostId AND r.status = 'PENDING'")
    List<Reservation> getAllPendingReservationRequestsByHost(@Param("hostId") Integer hostId);

    @Query("SELECT r FROM Reservation r WHERE r.accommodation.id IN :ids")
